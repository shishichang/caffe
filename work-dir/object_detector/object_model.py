...
#            print(ctg_detail)
    ...
    def demo_image(self):
        ...
        for i in range(numbox):
            ymin = int(results[i]['y1'] * h)
            xmin = int(results[i]['x1'] * w)
            ymax = int(results[i]['y2'] * h)
            xmax = int(results[i]['x2'] * w)
            ...
            #               '{:s} | {:.3f}'.format(results[i]['class'], results[i]['score']),
            #               bbox=dict(color, alpha=0.5),
